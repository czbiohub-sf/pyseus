...
def compute_stoich_df(m_imputed, seq_df, rnaseq, pvals, pull_uni,
    ...
    divide_by_bait functions to return a dataframe with calculated stoich columns.
        ...
        metadata: DataFrame, protein group – Uniprot metadata table generated by Keith’s script
    ...
    for bait in baits:
        ...
        if key in association.keys():
            ...
            gene = gene.split('_')[0]
            select = m_imputed[
                m_imputed[('Info', 'Gene names')].apply(
                    lambda x: gene in str(x).split(';'))]['Info', 'Protein IDs']
            if select.shape[0] > 0:
